...
class UserSearchResultsPage extends Component {
    ...
    render() {
        if(!this.state.ready){
            return <h1 className="text-center pt-5 mt-5">Chờ tí...</h1>
        }
        ...
        if (friendsArrLength > 0) {
            friends = (
                <Fragment>
                    <h3 className="mt-5">Danh sách bạn bè</h3>
                    ...
                    {this.props.friendsArrSearch.map((friend) =>
                        <Friend
                            ...
                            secondButtonLink={`/`}
                            firstButtonText={'Trang cá nhân'}
                            secondButtonText={'Xóa kết bạn'}
        ...
        if (requestLength > 0) {
            requests = (
                <Fragment>
                    <h3 className="mt-5">Người này đã gửi yêu cầu kết bạn</h3>
                    <div className="hr-styles"></div>
                    {this.props.userWaitingForAcceptingRequest.map((friend) =>
                        <FriendRequest
                            ...
                            {...this.props}
                            {...friend}
                            firstButtonText={'Đồng ý'}
                            secondButtonText={'Từ chối'}
                            thirdButtonText={'Xem trang cá nhân'}
        ...
        if (waitingForResponseUsers > 0) {
            friendsCandidates = (
                <Fragment>
                    <h3 className="mt-5">Yêu cầu đang đợi phản hồi</h3>
                    <div className="hr-styles"></div>
                    {
                        this.props.usersReceivedRequestFromCurrentUser.map((friend) =>
                            <Friend
                                ...
                                firstButtonLink={`/home/profile/${friend.id}`}
                                secondButtonLink={`/`}
                                firstButtonText={'Xem trang cá nhân'}
                                secondButtonText={'Hủy yêu cầu kết bạn'}
                                secondButtonOnClick={this.rejectRequest}
                            />)
                    }
                </Fragment>
        ...
        if (friendsCandidatesArr > 0) {
            remainCandidates = (
                <Fragment>
                    <h3 className="mt-5">Có thể bạn biết</h3>
                    <div className="hr-styles"></div>
                    {
        ...
        if (!friends && !requests && !friendsCandidates && !remainCandidates && !this.props.searchResults.loading) {
            noResult = (
                <Fragment>
                    <h4>Không tìm thấy kết quả với <span className="App-secondary-color">"{this.props.search}"</span></h4>
                    <div className="hr-styles"></div>
                </Fragment>
            )
        }
                ...
                <article className="main-article-shared-content">
                    <section className="friend-content-section">
                        <div className="container col-md-12 text-center mb-5">
                            <h2 className="text-center font-weight-bold mt-4" style={{ 'margin': '1rem auto' }}>Kết quả tìm kiếm</h2>
                            <div className="hr-styles"></div>
